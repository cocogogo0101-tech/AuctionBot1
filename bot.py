...
from database import init_db, set_setting, get_setting, all_settings
from security import is_allowed_guild, has_auction_role, verify_secret
from auctions import create_auction as _create_auction_record  # not used; auctions.create_auction used
...
from database import create_auction, get_active_auction
...
async def on_ready():
    ...
    # init DB
    await init_db()
    # leave other guilds if server_id is set
    ...
    try:
        # sync commands to guild if server configured (faster)
        server_id = await get_allowed_server_id()
        guild_obj = discord.Object(id=server_id) if server_id else None
...
@app_commands.describe(server="Server to allow (use current server)", secret="Secret code (for exclusive actions)")
async def config_set_server(interaction: discord.Interaction, secret: str = ""):
    # only allow from inside guild and allow only if secret matches or user has manage_guild
    if interaction.guild is None:
        await interaction.response.send_message("Execute this command in the server you want to allow.", ephemeral=True)
        return
    # optional: require admin permission or secret
    if not interaction.user.guild_permissions.manage_guild and secret != (await get_setting("secret_code") or ""):
...
async def auction_open(interaction: discord.Interaction, start_bid: str, min_increment: str, duration_minutes: int = DEFAULT_AUCTION_DURATION_MIN, secret: str = ""):
    ...
    if not (role_ok or interaction.user.guild_permissions.manage_guild or secret == (await get_setting("secret_code") or "")):
...
async def auction_end(interaction: discord.Interaction):
    # permission: role or manage_guild
    role_id = await get_setting("role_id")
    role_ok = False
    if role_id:
...
async def auction_undo_last(interaction: discord.Interaction):
    ...
    if not (role_ok or interaction.user.guild_permissions.manage_guild):
        await interaction.response.send_message("You don't have permission.", ephemeral=True)
        return
    from database import get_active_auction, undo_last_bid
    active = await get_active_auction()
    if not active:
...
async def auction_reset(interaction: discord.Interaction, secret: str):
    # require secret
    if secret != (await get_setting("secret_code") or "") and not interaction.user.guild_permissions.manage_guild:
    ...
    await end_auction(active["id"], active.get("start_bid"), None)
...
async def on_interaction(interaction: discord.Interaction):
    try:
        cid = getattr(interaction, "data", {}).get("custom_id", "")
        if cid and cid.startswith("bid_"):
            ...
            if len(parts) >= 3:
                ...
                if typ == "500k":
                    ...
                    return
                if typ == "custom":
                    modal = __import__("auctions").auctions.BidModal(auction_id)
